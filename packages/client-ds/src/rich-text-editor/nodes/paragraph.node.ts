...
export const ParagraphNode = {
  ...
  renderTextNode: <Message>({ node, index, start, selection, markClassNameForMarks }: RenderRichTextNodeOptions): ReadonlyArray<Html | string> => {
    ...
    return ranges.map((range) => {
      const attrs = [
        Class(`${markClassName || (range.selected ? "" : "contents")} ${range.selected ? "rounded-sm bg-primary/20 text-foreground" : ""}`),
